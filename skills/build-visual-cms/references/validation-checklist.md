...
- 공개 이미지 그리드가 3열이면 편집기 개요도 3열인지처럼 구현 범위의 열 수를 직접 대조한다. 편집기 폭만을 이유로 열 수가 줄지 않았는지 확인한다.
...
- 이미지 중심 컬렉션의 모든 축약 카드에 현재 썸네일과 캡션이 보이고 입력창이 상시 반복되지 않는지 확인한다.
- 카드를 선택하면 그 카드가 포함된 행 바로 다음에 전체 폭 상세 영역이 열리고, 같은 카드를 다시 선택하면 닫히며, 다른 카드를 선택하면 새 카드가 포함된 행 다음으로 이동하는지 확인한다.
...
- 예시·초기 콘텐츠를 만들거나 교정했다면 구현 범위 전체에서 이미지, 제목, 캡션과 대체 텍스트가 실제 자산과 의미상 일치하는지 확인한다. 기존 운영 콘텐츠의 불일치를 승인 없이 덮어쓰지 않았는지 확인한다.
...
- 드래그 손잡이에 반복되는 시각적 `순서 변경` 문구 없이 아이콘, 접근 가능한 이름과 tooltip이 있는지 확인한다.
- 그리드에서 포인터나 키보드가 cell 사이를 이동할 때 주변 카드와 placeholder가 예상 최종 순서대로 즉시 재배치되고 미리보기도 같은 임시 순서를 보이는지 확인한다. drop 전 보인 순서와 확정 순서가 같아야 하며 취소·영역 밖 drop·유효하지 않은 drop에서는 원래 순서와 선택을 복원해야 한다.
